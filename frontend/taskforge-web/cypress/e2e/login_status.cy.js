describe("TaskForge user login status", () => {
  ...
  beforeEach(function () {
    ...
    if (!email || !password) {
      cy.log("Set CYPRESS_EMAIL and CYPRESS_PASSWORD to run login-status tests.");
      this.skip();
    }

    userId = null;
    const timestamp = Date.now();
    fullName = `Cypress Login Status User ${timestamp}`;
    userEmail = `cypress.login.status.${timestamp}@taskforge.test`;
  ...
  it("changes a user login status from Active to Inactive", () => {
    cy.reload();
    ...
    cy.intercept("PUT", `**/api/Users/${userId}`).as("deactivateUser");
